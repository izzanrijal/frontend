...
const fetchChartData = async () => {
  ...
  try {
    const response = await apiService.get('/student/analys/example-chart/result', {
    ...
    results.value = data.userScores.map(score => ({
      label: score.tryout,
      value: `${score.score}% (${parseFloat(score.score) >= 65.0 ? 'Memenuhi Nilai Batas Kelulusan' : 'Belum Memenuhi Nilai Batas Kelulusan'})`,
      color: parseFloat(score.score) >= 65.0 ? '#005BC5' : '#FFA39E',
      passed: parseFloat(score.score) >= 65.0
    }));
